...
export default function StorekeeperRequestsScreen() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      console.error(error);
      handleApiError(error, 'Khong the tai danh sach yeu cau');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      <Pressable onPress={() => router.push(`/storekeeper/request-detail?id=${item.id}`)}>
        <View className="flex-row justify-between items-start mb-3">
          ...
          <View className="bg-yellow-50 px-2.5 py-1 rounded-full border border-yellow-100">
            <Text className="text-yellow-600 font-bold text-[10px]">CHO DUYET</Text>
          </View>
        </View>
        ...
        <View className="border-t border-b border-gray-50 py-3 my-1">
          ...
          <View className="flex-row items-center">
            ...
            <Text className="text-xs text-gray-600 ml-2">
              Han muon: {new Date(item.start_date).toLocaleDateString('vi-VN')} - {new Date(item.due_date).toLocaleDateString('vi-VN')}
            </Text>
          </View>
        </View>
        ...
        {item.notes && (
          <View className="mt-2 bg-gray-50 p-2.5 rounded-lg border border-gray-100">
            <Text className="text-[10px] font-bold text-gray-400 uppercase">Ly do muon</Text>
        ...
        <View className="flex-row justify-end mt-3 pt-2">
          <View className="flex-row items-center">
            <Text className="text-primary font-bold text-xs mr-1">Xem chi tiet</Text>
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white pt-16 pb-4 px-6 border-b border-gray-100 flex-row items-center shadow-sm">
        ...
        <Text className="text-xl font-bold text-gray-900">Yeu cau cho duyet</Text>
      </View>
      ...
      <View className="px-6 pt-4 pb-2">
        <View className="flex-row items-center bg-white border border-gray-200 rounded-xl px-3 py-2.5 shadow-sm">
          ...
          <TextInput
            ...
            placeholder="Tim kiem nguoi muon hoac thiet bi..."
            value={searchQuery}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center justify-center py-20 bg-white rounded-2xl mx-6 border border-gray-100 p-6 shadow-sm">
            <Feather name="check-square" size={48} color="#D1D1D6" />
            <Text className="text-gray-400 mt-4 font-bold text-base">Khong co yeu cau cho duyet</Text>
            <Text className="text-gray-400 text-xs text-center mt-1">Tat ca cac don muon da duoc xu ly hoan tat.</Text>
          </View>
        }
      />
    </View>
